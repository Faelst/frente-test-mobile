...
→ Retorna { "token": "...", "name": "Ash", "email": "ash@pokedash.com" }

Token armazenado no AsyncStorage
Ao abrir o app novamente, o usuário permanece logado.
...
const res = await api.get(`/fetch-skills-by-pokemon-name-order-by-skill-name/${name}`);
return res.data;
};
